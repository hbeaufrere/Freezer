    ...
    try:
        db.execute(
            ...
            (data['box_id'], data['row_pos'], data['col_pos'],
             ...
             data.get('date_stored'), data.get('freeze_thaw_cycles', 0))
        )
        db.commit()
    ...
    try:
        db.execute(
            """UPDATE research_tubes
               SET sample_id = ?, description = ?, date_stored = ?,
                   freeze_thaw_cycles = ?, updated_at = datetime('now')
               WHERE id = ?""",
            (data.get('sample_id', ''), data.get('description', ''),
             data.get('date_stored'), data.get('freeze_thaw_cycles', 0), tube_id)
        )
        db.commit()
        tube = db.execute("SELECT * FROM research_tubes WHERE id = ?", (tube_id,)).fetchone()
        if not tube:
...
def record_thaw(tube_id):
    ...
    db.execute(
        """INSERT INTO retrieval_log (section, tube_identifier, tube_info, action, retrieved_by, purpose)
           VALUES (?, ?, ?, ?, ?, ?)""",
        ('research', tube['sample_id'] or f'#{tube_id}', location, 'thawed', retrieved_by, purpose)
    )
...
def delete_tube(tube_id):
    ...
    if tube:
        ...
        db.execute(
            """INSERT INTO retrieval_log (section, tube_identifier, tube_info, action, retrieved_by, purpose)
               VALUES (?, ?, ?, ?, ?, ?)""",
            ('research', tube['sample_id'] or f'#{tube_id}', location, 'removed', retrieved_by, purpose)
        )
